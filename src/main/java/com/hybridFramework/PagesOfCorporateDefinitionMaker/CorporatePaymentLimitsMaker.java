...
public class CorporatePaymentLimitsMaker {
	static WebDriver driver;

	private final static Logger log = LoggerHelper.getLogger(CorporatePaymentLimitsMaker.class);
	...
	@FindBy(xpath = "//*[text()='Corporate Payment Limits Maker']//parent::a")
	WebElement CorporatPaymentLimitsMaker;
	@FindBy(xpath = "//*[text()='Fields in ']")
	WebElement Fieldsareinmandatory;
	@FindBy(xpath = "//input[@name='maxTxnLimit']")
	WebElement maxTxnLimit;
	@FindBy(xpath = "//input[@name='perTxnMinLimit']")
	WebElement perTxnMinLimit;
	@FindBy(xpath = "//input[@name='perTxnMaxLimit']")
	WebElement perTxnMaxLimit;

	@FindBy(xpath = "//select[@name='isActive']")
	WebElement isActiveDailPaymentLimits;
	...
	@FindBy(xpath = "//*[text()='Authoriser Corporate Payment Limits']//parent::a")
	WebElement AuthoriseCorporatePaymentLimits;
	@FindBy(xpath = "//td[contains(text(),'Corporate Payment Limits Checker')]")
	WebElement verifyCorporatePaymentLimitsChecker;
...
// Modify Page in Maker
	...
	@FindBy(xpath = "//*[text()='Modify Corporate Payment Limits Maker']")
	WebElement verifyModifyCorporatePaymentLimitsMakerPage;
...
//	WebElement selectmandateCodeInModifyPage;
	...
	@FindBy(xpath = "//form[@name='modifyEntityMandateLimitsMaker']//table//tr[1]//input[@name='Submit']")
	WebElement modifysubmitbutton;
	@FindBy(xpath = "//form[@name='grpFeeModifyMaker']//table[@class='listingtable']//tr//td[1]")
	...
	public CorporatePaymentLimitsMaker(WebDriver driver) {
		this.driver = driver;
...
// click on Corporate Payment Limits Maker SubMenu
	public void clickonCorporatPaymentLimitsMakerSubMenu() {
		log.info("click on Corporate Payment Limits Maker SubMenu ==>>" + CorporatPaymentLimitsMaker.getText());
		CorporatPaymentLimitsMaker.click();
	}
...
// to switch the Add Country Maker page.
	...
	String MinmTxnLimit = "1";

	static String getmaxTxnLimit;
	static String getperTxnMinLimit;
	static String getperTxnMaxLimit;
	static String getisActiveDailyPaymentLimits;
	...
	// Payment Code Or Mandate Code
	public void selectPaymentCodeDropDown() throws Exception {
		//
		String[] paymentCode = { "BKASH", "hh" };
		// Thread.sleep(3000);
		System.out.println("Get the Text of String [] Array Before ==> " + paymentCode);
		paymentCodeList = Arrays.asList(paymentCode);
		StringValueOfpaymentCode = String.join(",", paymentCodeList);
		System.out.println("Get String Value of Payment Code code" + StringValueOfpaymentCode);

		methodofPaymentCode(driver, StringValueOfpaymentCode);

	}

	public static void methodofPaymentCode(WebDriver driver, String value) throws Exception {
		int j = 0;
		Thread.sleep(5000);
		Select selectPaymentCode = new Select(driver.findElement(By.xpath("//select[@name='mandateCode']")));
		List<WebElement> listOfPaymentCode = selectPaymentCode.getOptions();
		int getSizeOfPaymentCode = listOfPaymentCode.size();
		System.out.println("Get Size Of Payment Code ==>" + getSizeOfPaymentCode);

		for (int i = 0; i < listOfPaymentCode.size(); i++) {
			GetTextOfPaymentCode = listOfPaymentCode.get(i).getText();
			System.out.println("Get Text Of Payment Code ==>" + GetTextOfPaymentCode);

			for (String w : value.trim().split(",")) {
				// System.out.println("Get String values ==>>" + w);
				x = w.toString();
				System.out.println("global variable declearation ==>" + x);
				if (x.equals(GetTextOfPaymentCode)) {
					System.out.println("Text has Matched with String Array and Drop Down Values ==>" + x);

					// System.out.println("Get String values Matched with value and selected ==>>" +
					// w.toString());
					selectPaymentCode.selectByVisibleText(GetTextOfPaymentCode);
					log.info("drop down has selected with Value ==>" + x);

					break;
				}

				else {
					System.out.println("Text has NOT Matched with String Array and Drop Down Values ==>" + x);
				}
				break;
			}
	...
	public void enterDailymaxTxnLimit(String maxTxnLimit) {
		log.info("Enter Daily maximum  Transaction Limit ==>" + maxTxnLimit);
		this.maxTxnLimit.sendKeys(maxTxnLimit);
		getmaxTxnLimit = maxTxnLimit.toString();
		System.out.println("Get Daily maximum  Transaction Limit ==>" + getmaxTxnLimit);
	}

	public void enterPerTransactionMinmAmountLimit(String perTxnMinLimit) {
		log.info("Enter Per Transaction  Minimum Amount Limit ==>" + perTxnMinLimit);
		this.perTxnMinLimit.sendKeys(perTxnMinLimit);
		getperTxnMinLimit = perTxnMinLimit.toString();
		System.out.println("Enter Per Transaction action Minimum Amount Limit ==>" + getperTxnMinLimit);
	}

	public void enterPerTransactionMaximumAmountLimit(String perTxnMaxLimit) {
		log.info("Enter Per Transaction Minimum Amount Limit ==>" + perTxnMaxLimit);
		this.perTxnMaxLimit.sendKeys(perTxnMaxLimit);
		getperTxnMaxLimit = perTxnMaxLimit.toString();
		System.out.println("Enter Per Transaction Maximum Amount Limit ==>" + getperTxnMaxLimit);
	}

// select  is Active Daily Payment Limits Maker
	public void selectisActiveDailyPaymentLimits() {

		String active = "Yes";
		log.info("select is Active Daily Payment Limits ? ==>");
		Select aa = new Select(isActiveDailPaymentLimits);
		aa.selectByVisibleText(active);
		String getisActive = active.toString();
		System.out.println("get is Active Daily Payment Limits==>" + getisActive);
		String[] spl = getisActive.split("es");
		System.out.println("Split Array Values of Yes ===>>" + spl);
		List<String> splitList1 = Arrays.asList(spl);
		getisActiveDailyPaymentLimits = String.join("", splitList1);
		System.out.println(
				"get is Active Daily Payment Limits after Split (Yes) As Y==>" + getisActiveDailyPaymentLimits);
...
// click on Corporate Definition Checker Menu
	...
	// click On Authorise Corporate Payment Limits SubMenu
	public void clickOnAuthoriseCorporatePaymentLimitsSubMenu() {
		log.info(
				"click On Authorise Corporate Payment Limits Sub Menu ==>" + AuthoriseCorporatePaymentLimits.getText());
		AuthoriseCorporatePaymentLimits.click();
	}

	public void verifyCorporatePaymentLimitsCheckerPage() throws Exception {
		Thread.sleep(3000);
		List<WebElement> iframeElements = driver.findElements(By.tagName("iframe"));
		System.out.println(iframeElements);
		...
		log.info("verify Corporate Payment Limits Checker ==>" + verifyCorporatePaymentLimitsChecker.getText());
		String CorporatePaymentLimitsChecker = verifyCorporatePaymentLimitsChecker.getText();

		System.out.println("Get text of Corporate Payment Limits Checker page ==>" + CorporatePaymentLimitsChecker);
		Assert.assertEquals("CORPORATE PAYMENT LIMITS CHECKER", CorporatePaymentLimitsChecker);
	}
	...
	public void verifyChekerPageByTextWhenAddedMaker() throws Exception {
		...
		String EntityType = "//table[@id='list']//tbody/tr/td[6]";
		String CountryCode = "//table[@id='list']//tbody/tr/td[7]";
		String PaymentCode = "//table[@id='list']//tbody/tr/td[8]";
		String PaymentId = "//table[@id='list']//tbody/tr/td[9]";
		String MinmTxnLimit = "//table[@id='list']//tbody/tr/td[10]";
		String DailymaxTxnLimit = "//table[@id='list']//tbody/tr/td[11]";
		String PerTransactionMinmAmountLimit = "//table[@id='list']//tbody/tr/td[12]";
		String PerTransactionMaximumAmountLimit = "//table[@id='list']//tbody/tr/td[13]";
		...
		toverifyCheckerlistByUsingArrayList(EntityType, splitvaluesofCorporateIDAsEntityType);
		toverifyCheckerlistByUsingArrayList(CountryCode, CountryCode1);
		toverifyCheckerlistByUsingArrayList(PaymentCode, x);
		toverifyCheckerlistByUsingArrayList(MinmTxnLimit, MinmTxnLimit);
		toverifyCheckerlistByUsingArrayList(DailymaxTxnLimit, getmaxTxnLimit);
		toverifyCheckerlistByUsingArrayList(PerTransactionMinmAmountLimit, getperTxnMinLimit);
		toverifyCheckerlistByUsingArrayList(PerTransactionMaximumAmountLimit, getperTxnMaxLimit);
		toverifyCheckerlistByUsingArrayList(IsActive, getisActiveDailyPaymentLimits);
...
// refresh Method Should be use when the when Search Text is not Remove and Click on select All  values.
	public void verifyViewpageBySearchField() throws InterruptedException {
		...
		this.searchfield.clear();
		log.info("search By Get Text Of Payment Code ==>" + x);
		this.searchfield.sendKeys(x);
		Thread.sleep(4000);
		this.searchfield.clear();
...
// x-path of view page and calling their respective variable  and verify
	public void VerifyViewPageByGetText() {
		...
		String PaymentCode = "//tbody[@role='alert']//tr//td[5]";
		String PaymentID = "//tbody[@role='alert']//tr//td[6]";
		String MinTransactionLimit = "//tbody[@role='alert']//tr//td[7]";
		String MaxDailyTransactionLimit = "//tbody[@role='alert']//tr//td[8]";
		String perTxnMinLimit = "//tbody[@role='alert']//tr//td[9]";
		String perTxnMaxmLimit = "//tbody[@role='alert']//tr//td[10]";
		...
		verifyTextOnViewPage(PaymentCode, x);
		verifyTextOnViewPage(PaymentID, "");
		verifyTextOnViewPage(MinTransactionLimit, MinmTxnLimit);
		verifyTextOnViewPage(MaxDailyTransactionLimit, getmaxTxnLimit);
		verifyTextOnViewPage(perTxnMinLimit, getperTxnMinLimit);
		verifyTextOnViewPage(perTxnMaxmLimit, getperTxnMaxLimit);
		verifyTextOnViewPage(IsActive, getisActiveDailyPaymentLimits);
...
// Select  All Drop down values in Maker view page when the Added Corporate Pool Account has Approved By Checker 
	public void selectAllDropDownvaluesOneByOneOnViewPage() {
		...
		WebElement selectPaymentCode = driver.findElement(By.xpath("//tfoot//tr//th[5]//select"));
		WebElement selectPaymentID = driver.findElement(By.xpath("//tfoot//tr//th[6]//select"));
		WebElement selectMinTransactionLimit = driver.findElement(By.xpath("//tfoot//tr//th[7]//select"));
		WebElement selectMaxDailyTransactionLimit = driver.findElement(By.xpath("//tfoot//tr//th[8]//select"));
		WebElement selectperTxnMinLimit = driver.findElement(By.xpath("//tfoot//tr//th[9]//select"));
		WebElement selectperTxnMaxmLimit = driver.findElement(By.xpath("//tfoot//tr//th[10]//select"));
		...
		verifyBySelectDropDown(selectPaymentCode, x);
		verifyBySelectDropDown(selectPaymentID, "");
		verifyBySelectDropDown(selectMinTransactionLimit, MinmTxnLimit);
		verifyBySelectDropDown(selectMaxDailyTransactionLimit, getmaxTxnLimit);
		verifyBySelectDropDown(selectperTxnMinLimit, getperTxnMinLimit);
		verifyBySelectDropDown(selectperTxnMaxmLimit, getperTxnMaxLimit);
		verifyBySelectDropDown(selectIsActive, getisActiveDailyPaymentLimits);
...
// select one by one of Select in View page of Maker
	...
	public void clickOnModifyButtonAndverifyTheModifiedPage() {
		...
		log.info(" Get the Text of Modify page ==>" + verifyModifyCorporatePaymentLimitsMakerPage.getText());
		Assert.assertEquals(verifyModifyCorporatePaymentLimitsMakerPage.getText(),
				"MODIFY CORPORATE PAYMENT LIMITS MAKER");
	}
...
// //form[@name='grpFeeModifyMaker']//table[@class='listingtable']//tr//td[1] for click on link
	// form[@name='grpFeeModifyMaker']//table[@class='listingtable']//tr//td[2] for
	// Max Trxn limit
	...
	public void clickOnLinkOnModifyPageWhenModify() {
		...
		String DailyMaximumTxcLimit = "//form[@name='grpFeeModifyMaker']//table[@class='listingtable']//tr//td[2]";
		clickOnLinkMethod(DailyMaximumTxcLimit, getmaxTxnLimit);
	}
	...
	public void clickOnLinkMethod(String xpathvalue, String value) {
		...
		for (int i = 0; i < web.size(); i++) {
			...
			if (name.contains(value)) {
				...
				driver.findElements(
						By.xpath("//form[@name='grpFeeModifyMaker']//table[@class='listingtable']//tr//td[1]")).get(i)
						.click();
	...
	public void verifyCheckerPageModificationhasDoneByMaker() throws InterruptedException {
		...
		toverifyListOfValuesOnCheckerPageWhenModified(EntityType, splitvaluesofCorporateIDAsEntityType);
		toverifyListOfValuesOnCheckerPageWhenModified(CountryCode, CountryCode1);
		toverifyListOfValuesOnCheckerPageWhenModified(PaymentCode, x);
		toverifyListOfValuesOnCheckerPageWhenModified(MinmTxnLimit, MinmTxnLimit);
		toverifyListOfValuesOnCheckerPageWhenModified(DailymaxTxnLimit, getmaxTxnLimit);
		toverifyListOfValuesOnCheckerPageWhenModified(PerTransactionMinmAmountLimit, getperTxnMinLimit);
		toverifyListOfValuesOnCheckerPageWhenModified(PerTransactionMaximumAmountLimit, getperTxnMaxLimit);
		toverifyListOfValuesOnCheckerPageWhenModified(IsActive, getisActiveDailyPaymentLimits);
...
//refresh Method Should be use when the when Search Text is not Remove and Click on select All  values.
	public void verifyViewpageBySearchFieldWhenModify() throws InterruptedException {
		String[] search = { CountryCode1, splitvaluesofCorporateIDAsEntityID, x, getperTxnMinLimit };
...
//x-path of view page and calling their respective variable  and verify
	public void toVerifyByGetTextInViewPageWhenModify() {
		...
		verifyViewPageWhenModify(PaymentCode, x);
		verifyViewPageWhenModify(PaymentID, "");
		verifyViewPageWhenModify(MinTransactionLimit, MinmTxnLimit);
		verifyViewPageWhenModify(MaxDailyTransactionLimit, getmaxTxnLimit);
		verifyViewPageWhenModify(perTxnMinLimit, getperTxnMinLimit);
		verifyViewPageWhenModify(perTxnMaxmLimit, getperTxnMaxLimit);
		verifyViewPageWhenModify(IsActive, getisActiveDailyPaymentLimits);
...
//  5 drop down are available on the 
	public void toVerifytheModifyDataInMakerViewPagByUsingDropDown() {
		...
		verifyViewPPageBySelectDropDownWhenModified(selectPaymentCode, x);
		verifyViewPPageBySelectDropDownWhenModified(selectPaymentID, "");
		verifyViewPPageBySelectDropDownWhenModified(selectMinTransactionLimit, MinmTxnLimit);
		verifyViewPPageBySelectDropDownWhenModified(selectMaxDailyTransactionLimit, getmaxTxnLimit);
		verifyViewPPageBySelectDropDownWhenModified(selectperTxnMinLimit, getperTxnMinLimit);
		verifyViewPPageBySelectDropDownWhenModified(selectperTxnMaxmLimit, getperTxnMaxLimit);
		verifyViewPPageBySelectDropDownWhenModified(selectIsActive, getisActiveDailyPaymentLimits);
