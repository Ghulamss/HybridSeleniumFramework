...
public class CorporateCountryLicenseMaker {
	static WebDriver driver;

	private final static Logger log = LoggerHelper.getLogger(CorporateCountryLicenseMaker.class);
	...
	@FindBy(xpath = "//*[text()='Corporate Country License Maker']//parent::a")
	WebElement CorporateCountryLicenseMaker;
	@FindBy(xpath = "//*[text()='Fields in ']")
	WebElement Fieldsareinmandatory;
	@FindBy(xpath = "//select[@name='licenseType']")
	WebElement selectlicenseType;
	...
	@FindBy(xpath = "//input[@name='agreementLabel']")
	WebElement agreementLabel;
	@FindBy(xpath = "//select[@id='aFreq']")
	WebElement selectAlertFrequency;
	@FindBy(xpath = "//input[@name='aMailId']")
	WebElement alertMail;
	@FindBy(xpath = "//input[@name='gracePeriod']")
	WebElement gracePeriodInDays;
...
// Modify Page in Maker
	...
	public CorporateCountryLicenseMaker(WebDriver driver) {
		this.driver = driver;
...
// click on Corporate Country License Maker SubMenu
	public void clickonCorporateCountryLicenseMakerSubMenu() {
		log.info("click on Corporate Country License Maker SubMenu ==>>" + CorporateCountryLicenseMaker.getText());
		CorporateCountryLicenseMaker.click();
	}
...
// to switch the Add Country Maker page.
	...
	public String selectlicenseTypeDropDown() {
		String licenseType = "Agreement Type";
		getlicenseType = licenseType.toString();
		System.out.println("get license Type Value ==>" + getlicenseType);
		log.info("select license Type Drop Down ==>");
		Select selectlicense = new Select(selectlicenseType);
		selectlicense.selectByVisibleText(licenseType);
		return licenseType;
	}
...
//  Switch The Window From  2 Child(After Modified Data ) to Parent Child.
	...
	public void enteragreementLabel(String agreementLabel) {
		log.info(" enter agreement Label ===>" + agreementLabel);
		this.agreementLabel.sendKeys(agreementLabel);
		getagreementLabel = agreementLabel.toString();
		System.out.println("get agreement Label value ==>" + getagreementLabel);

	}

	public void selectDateOfIssue() throws Exception {
		driver.findElement(By.xpath("//input[@name='dateOfIssue']")).click();
		log.info(" Click date Of Issue ==>");

		String aa = driver.findElement(By.xpath("//div[@class='titleText']//div")).getText();
		System.out.println(aa);
		while (!driver.findElement(By.xpath("//div[@class='header']//div[@class='titleText']//div")).getText()
				.contains("Sep 2020")) {
			// Thread.sleep(5000);
			Actions action = new Actions(driver);
			// moving to the main menu and then sub menu and clicking on it using object of
			// the Actions class
	...
	public void selectDateOfExpiry() throws Exception {
		driver.findElement(By.xpath("//input[@name='dateOfExpiry']")).click();
		log.info(" Click on date Of Expiry ==>");
	...
	public void selectAlertDate() throws Exception {
		...
		for (int i = 0; i < days.size(); i++) {
			String getTextOfDate = days.get(i).getText();
			System.out.println("Get All Text On days  Calender ==>" + getTextOfDate);
			if (days.get(i).getText().equals("6")) {
	...
	public void selectAlertFrequency() {
		String AlertFrequen = "Daily";
		getAlertFrequen = AlertFrequen.toString();
		System.out.println("get Alert Frequency Value ==>" + getAlertFrequen);
		log.info("select Alert Frequency ==>" + getAlertFrequen);
		Select AlertFrequency = new Select(selectAlertFrequency);
		AlertFrequency.selectByVisibleText(AlertFrequen);
	}

	public void enteralertMail(String alertMail) {
		log.info("enter alert Mail ==>" + alertMail);
		this.alertMail.sendKeys(alertMail);
		getalertMail = alertMail.toString();
		System.out.println("get alert Mail value ==>" + getalertMail);

	}

	public void entergracePeriodInDays(String gracePeriodInDays) {
		log.info("enter grace Period In Days ==>" + gracePeriodInDays);
		this.gracePeriodInDays.sendKeys(gracePeriodInDays);
		getgracePeriodInDays = gracePeriodInDays.toString();
		System.out.println("get grace Period In Days value ==>" + getgracePeriodInDays);
	}
...
// select  is Active Country ? in Entity License Country Maker
	public void selectisActiveEntityLicenseCountry() {
		...
		String s = "Yes";
		log.info("select is Active Entity License Country?==" + isActiveCountry);
		...
		System.out.println("get Is Active Entity License Country after Split (Yes) As Y==>" + getIsActiveCountry);
...
// select one by one of Select in View page of Maker
	...
	public void verifyTheModifiedPage() {
		...
		Assert.assertEquals(verifyMODIFYENTITYLICENSECOUNTRYMAKERPage.getText(), "MODIFY ENTITY LICENSE COUNTRY MAKER");
	}
...
//		
	...
	// Corporate Country
	public void verifyEntityLicenseCountryCheckerPageModificationhasDoneByMaker() throws InterruptedException {
		...
		toverifyListOfValuesOnCheckerPageWhenModifiedCorporateCountryLicense(EntityId,
				splitvaluesofCorporateIDAsEntityID);
		toverifyListOfValuesOnCheckerPageWhenModifiedCorporateCountryLicense(EntityName,
				splitvaluesofCorporateIDAsEntityType);
		...
		toverifyListOfValuesOnCheckerPageWhenModifiedCorporateCountryLicense(FileName,
				"Training - KYC & AML20200805022609.pdf");
