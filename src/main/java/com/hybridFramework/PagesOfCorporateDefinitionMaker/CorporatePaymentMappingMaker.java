...
public class CorporatePaymentMappingMaker {
	static WebDriver driver;

	private final static Logger log = LoggerHelper.getLogger(CorporatePaymentMappingMaker.class);
	...
	@FindBy(xpath = "//*[text()='Corporate Payment Mapping Maker']//parent::a")
	WebElement CorporatePaymentMappingMaker;
	@FindBy(xpath = "//*[text()='Fields in ']")
	WebElement Fieldsareinmandatory;

	
	@FindBy(xpath = "//select[@name='isActive']")
	WebElement isActivePaymentMapping;
	...
	@FindBy(xpath = "//*[text()='Authoriser Corporate Payment Mapping']//parent::a")
	WebElement AuthoriserCorporatePaymentMapping;
	@FindBy(xpath = "//td[contains(text(),'Corporate Payment Mapping Checker')]")
	WebElement verifyCorporatePaymentMappingChecker;
...
// Modify Page in Maker
	...
	@FindBy(xpath = "//*[text()='Modify Entity Country Payment Maker']")
	WebElement verifyModifyEntityCountryPaymentMakerPage;
	...
	@FindBy(xpath = "//form[@name='entityCountryMandate']//table//tr[11]//input[@name='Submit']")
	...
	public CorporatePaymentMappingMaker(WebDriver driver) {
		this.driver = driver;
...
// click on Corporate Payment Mapping Maker SubMenu
	public void clickonCorporatePaymentMappingMakerSubMenu() {
		log.info("click on Corporate Payment Mapping Maker SubMenu ==>>"
				+ CorporatePaymentMappingMaker.getText());
		CorporatePaymentMappingMaker.click();
	}
...
// to switch the Add Country Maker page.
   ...
   // Group Country Code Or Country Code 
	...
	public static void methodofGroupCountry(WebDriver driver, String value) throws Exception {
		int j = 0;
		Thread.sleep(5000);
		Select selectGroupCountry = new Select(driver.findElement(By.xpath("//select[@name='country']")));
...
// select  is Active Payment Mapping Maker
	public void selectisActivePaymentMapping() {
		...
		String active = "Yes";
		log.info("select is Active Payment Mapping ? ==>" );
		Select aa = new Select(isActivePaymentMapping);
		aa.selectByVisibleText(active);
		String getisActive = active.toString();
		System.out.println("get is Active Payment Mapping ===>>" + getisActive);
		String[] spl = getisActive.split("es");
		System.out.println("Split Array Values of Yes ===>>" + spl);
		List<String> splitList1 = Arrays.asList(spl);
		getisActivePaymentMapping = String.join("", splitList1);
		System.out.println(
				"get is Active Payment Mapping after Split (Yes) As Y==>" + getisActivePaymentMapping);
...
// click on Corporate Definition Checker Menu
	...
	// click On Authoriser Corporate Payment Mapping SubMenu
	public void clickOnAuthoriserCorporatePaymentMappingSubMenu() {
		log.info("click On Authoriser Corporate Payment Mapping Sub Menu ==>"
				+ AuthoriserCorporatePaymentMapping.getText());
		AuthoriserCorporatePaymentMapping.click();
	}

	public void verifyCorporatePaymentMappingCheckerPage() throws Exception {
		Thread.sleep(3000);
		List<WebElement> iframeElements = driver.findElements(By.tagName("iframe"));
		System.out.println(iframeElements);
		...
		log.info("verify verify Corporate Payment Mapping Checker ==>" + verifyCorporatePaymentMappingChecker.getText());
		String CorporatePaymentMappingChecker = verifyCorporatePaymentMappingChecker.getText();

		System.out.println("Get text of Corporate Payment Mapping Checker page ==>" + CorporatePaymentMappingChecker);
		Assert.assertEquals("CORPORATE PAYMENT MAPPING CHECKER", CorporatePaymentMappingChecker);
	}
	...
	public void verifyChekerPageByTextWhenAddedMaker() throws Exception {
		...
		String EntityDescrition = "//table[@id='list']//tbody/tr/td[6]";
		String CountryCode = "//table[@id='list']//tbody/tr/td[7]";
		String PaymentCode = "//table[@id='list']//tbody/tr/td[8]";
		String IsActive = "//table[@id='list']//tbody/tr/td[9]";
		...
		toverifyCheckerlistByUsingArrayList(EntityDescrition, splitvaluesofCorporateIDAsEntityType);
		toverifyCheckerlistByUsingArrayList(CountryCode, CountryCode1);
		toverifyCheckerlistByUsingArrayList(PaymentCode, x);
		toverifyCheckerlistByUsingArrayList(IsActive, getisActivePaymentMapping);
...
// x-path of view page and calling their respective variable  and verify
	public void VerifyViewPageByGetText() {
		...
		String CorporateId = "//tbody[@role='alert']//tr//td[4]";
		...
		verifyTextOnViewPage(CorporateId, splitvaluesofCorporateIDAsEntityID);
		verifyTextOnViewPage(PaymentCode, x);
		verifyTextOnViewPage(IsActive, getisActivePaymentMapping);
...
// Select  All Drop down values in Maker view page when the Added Corporate Pool Account has Approved By Checker 
	public void selectAllDropDownvaluesOneByOneOnViewPage() {
		...
		WebElement selectCorporateId = driver.findElement(By.xpath("//tfoot//tr//th[4]//select"));
		WebElement selectPaymentCode = driver.findElement(By.xpath("//tfoot//tr//th[5]//select"));
		WebElement IsActive = driver.findElement(By.xpath("//tfoot//tr//th[6]//select"));
		
		
		verifyBySelectDropDown(selectGroupId, GroupId);
		verifyBySelectDropDown(selectCountryCode,CountryCode1 );
		verifyBySelectDropDown(selectCorporateId, splitvaluesofCorporateIDAsEntityID);
		verifyBySelectDropDown(selectPaymentCode, x);
		verifyBySelectDropDown(IsActive, getisActivePaymentMapping);
...
// select one by one of Select in View page of Maker
	...
	public void verifyTheModifiedPage() {
		...
		log.info(" Get the Text of Modify page===>>" + verifyModifyEntityCountryPaymentMakerPage.getText());
		Assert.assertEquals(verifyModifyEntityCountryPaymentMakerPage.getText(),
				"MODIFY ENTITY COUNTRY PAYMENT MAKER");
	}
	...
	public void selectPaymentCodeDropDownInModifyPage() {
		log.info("select Payment Code Drop Down In Modify Page ==>");
		Select PaymentCodeInModify = new Select(selectmandateCodeInModifyPage);
		PaymentCodeInModify.selectByVisibleText(x);
	}
	...
	toverifyListOfValuesOnCheckerPageWhenModified(EntityDescrition, splitvaluesofCorporateIDAsEntityType);
	...
	toverifyListOfValuesOnCheckerPageWhenModified(IsActive, getisActivePaymentMapping);
...
//refresh Method Should be use when the when Search Text is not Remove and Click on select All  values.
	public void verifyViewpageBySearchFieldWhenModify() throws InterruptedException {
		String[] search = { GroupId, splitvaluesofCorporateIDAsEntityID, x, getisActivePaymentMapping };
...
//x-path of view page and calling their respective variable  and verify
	public void toVerifyByGetTextInViewPageWhenModify() {
		...
		verifyViewPageWhenModify(CorporateId, splitvaluesofCorporateIDAsEntityID);
		verifyViewPageWhenModify(PaymentCode, x);
		verifyViewPageWhenModify(IsActive, getisActivePaymentMapping);
...
//  5 drop down are available on the 
	public void toVerifytheModifyDataInMakerViewPagByUsingDropDown() {
		...
		verifyViewPPageBySelectDropDown(selectCorporateId, splitvaluesofCorporateIDAsEntityID);
		verifyViewPPageBySelectDropDown(selectPaymentCode, x);
		verifyViewPPageBySelectDropDown(IsActive, getisActivePaymentMapping);
