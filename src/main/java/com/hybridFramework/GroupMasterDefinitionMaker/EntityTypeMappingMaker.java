...
public class EntityTypeMappingMaker {
	static WebDriver driver;

	private final static Logger log = LoggerHelper.getLogger(EntityTypeMappingMaker.class);
	...
	@FindBy(xpath = "//*[text()='Entity Type Mapping Maker']//parent::a")
	WebElement EntityTypeMappingMaker;
...
// Statrted  Checker X-path
	...
	@FindBy(xpath = "//*[text()='Authoriser Entity Type Mapping']")
	WebElement AuthoriserEntityTypeMapping;
	@FindBy(xpath = "//td[contains(text(),' Group Country Entity Type Checker')]")
	WebElement verifyGroupCountryEntityTypeChecker;
...
// Modify Page in Maker
	...
	public EntityTypeMappingMaker(WebDriver driver) {
		this.driver = driver;
...
// click on Entity Type Mapping Maker SubMenu
	public void clickonEntityTypeMappingMakerSubMenu() {
		log.info("click on Entity Type Mapping Maker SubMenu===>" + EntityTypeMappingMaker.getText());
		EntityTypeMappingMaker.click();
	}
...
// to switch the Add Country Maker page.
	...
	public void selectthedropdown() throws Exception {
		...
		String[] itemsNeeded = { "1121", "1141", "1161" };
		Thread.sleep(5000);
	...
	public static void addItems(WebDriver driver, String[] itemsNeeded) throws Exception

	{

		int j = 0;
		Thread.sleep(5000);
		Select sel = new Select(driver.findElement(By.xpath("//select[@name='entityTypeId']")));
		List<WebElement> products = sel.getOptions();
		int aa = products.size();
		System.out.println("Get Si " + aa);
...

		{
...

			{
				...
				sel.selectByVisibleText("1121-109ak");
...
// verify Successful Message On Maker Page
	public void verifySuccessfulMessageOnMakerPageAndClickOnLogOut() {
		...
		if (verifytheSuccessfulMessage.equals("Record has been added successfully.")) {
			...
			driver.switchTo().defaultContent();
			driver.findElement(By.xpath("//b[text()='Logout']")).click();
		} else if (verifytheSuccessfulMessage.equals("This Entity Type maaping has already used.")) {
			log.info("This Entity Type maaping has already used.");
			driver.switchTo().defaultContent();
...
// click on Group Master Definition Checker Menu
	...
	// click On Authoriser Entity Type Mapping SubMenu
	public void clickOnAuthoriserEntityTypeMappingSubMenu() {
		log.info("click On Authoriser Entity Type Mapping SubMenu===>>" + AuthoriserEntityTypeMapping.getText());
		AuthoriserEntityTypeMapping.click();
	}

	public void verifyGroupCountryEntityTypeCheckerPages() throws Exception {
		Thread.sleep(3000);
		List<WebElement> iframeElements = driver.findElements(By.tagName("iframe"));
		System.out.println(iframeElements);
		...
		log.info("verify Group Country Entity Type Checker===>>" + verifyGroupCountryEntityTypeChecker.getText());
		String GroupCountryEntityTypeChecker = verifyGroupCountryEntityTypeChecker.getText();

		System.out.println("Get text of Group Country Entity Type Checker page===>>" + GroupCountryEntityTypeChecker);
		Assert.assertEquals("GROUP COUNTRY ENTITY TYPE CHECKER", GroupCountryEntityTypeChecker);
	}
...
// Calling the Generic Method to verify the List in Country Checker page.
	...
	public void verifyAddcheckerListByUsingArraysListAnd() throws Exception {

//		String[] groupid = { "RGEX", "hhj" };
//		List<String> groupidList = Arrays.asList(groupid);
//		System.out.println("group id List===> " + groupidList);
//
//		String[] CountryCodeArray = { "" };
//		List<String> CountryCodeList = Arrays.asList(CountryCodeArray);
//		System.out.println("Country Code List===> " + CountryCodeList);
//
//		String[] entityIdArray = { "1204", "hhj" };
//		List<String> entityIdList = Arrays.asList(entityIdArray);
//		System.out.println("entity Id List===> " + entityIdList);
//
//		String[] IsActiveArray = { "Y", "Y" };
//		List<String> IsActiveList = Arrays.asList(IsActiveArray);
//		System.out.println("Is Active List===> " + IsActiveList);
		...
		toverifyCheckerlistByUsingArrayList(EntityTypeID, splitvaluesofCorporateDropDownValues);
...
//Calling the Generic Method to verify the List in Country Checker page.
	...
	for (int i = 0; i < web.size(); i++) {
		...
		if (name.equals(splitvaluesofCorporateDropDownValues)) {
			System.out.println("Get Text has Mtched with Unique values ==>"+splitvaluesofCorporateDropDownValues);
			Thread.sleep(3000);
			driver.findElements(By.xpath(
					//   //table[@id='list']//tbody/tr/td[4]//parent::td//preceding-sibling::td[3]//input[@type='checkbox']
					"//table[@id='list']//tbody/tr/td[4]//parent::td//preceding-sibling::td[3]//input[@type='checkbox']"))
					.get(i).click();
			log.info("Click On Check Box with Unique values ==>"+splitvaluesofCorporateDropDownValues);
		

		} else {
			System.out.println("Get Text has NOT Mtched with Unique values ==>"+splitvaluesofCorporateDropDownValues);
		}
	}
...
// refresh Method Should be use when the when Search Text is not Remove and Click on select All  values.
	public void verifyViewpageBySearchField()throws InterruptedException {
		...
		Thread.sleep(4000);
		this.searchfield.clear();
		log.info("search By Entity Type Code ==>"+splitvaluesofCorporateDropDownValues);
		this.searchfield.sendKeys(splitvaluesofCorporateDropDownValues);
		Thread.sleep(4000);
		this.searchfield.clear();
	...
	public void searchTheElementsByUsingArrayList() {

		String[] search = { "RGEX", "IN", "1006", "Y" };
	...
	public void searchTheElementsByUsingArray(WebDriver driver, List<String> value) {
...
//		System.out.println("group id List===> " + groupidList);
//
//		String[] CountryCodeArray = { "IN" };
//		List<String> CountryCodeList = Arrays.asList(CountryCodeArray);
//		System.out.println("Country Code List===> " + CountryCodeList);
//
//		String[] EntityDescriptionArray = { "PB Al ansari1", "Test Entity	" };
//		List<String> EntityDescriptioList = Arrays.asList(EntityDescriptionArray);
...
//		System.out.println("Is Active List===> " + IsActiveList);
		...
		String EntityDescription = "//tbody[@role='alert']//tr//td[4]";
		String EntityTypeId = "//tbody[@role='alert']//tr//td[5]";
		...
		verifyTextOnViewPage(EntityDescription, entitDescription);
		verifyTextOnViewPage(EntityTypeId, splitvaluesofCorporateDropDownValues);
		verifyTextOnViewPage(IsActive, getIsActiveCountry);
...
// Select  All Drop down values in Maker view page when the Added Country has Approved By Checker 
	public void selectAllvaluesOneByOne() {
		...
		WebElement selectEntityDescription = driver.findElement(By.xpath("//tfoot//tr//th[4]//select"));
		WebElement selectEntityTypeId = driver.findElement(By.xpath("//tfoot//tr//th[5]//select"));
		WebElement selectIsActive = driver.findElement(By.xpath("//tfoot//tr//th[6]//select"));

//		String[] groupid = { "RGEX" };
//		List<String> groupidList = Arrays.asList(groupid);
//		System.out.println("group id List===> " + groupidList);
//		String groupidString = String.join(",", groupidList);
//		System.out.println("Get String Value" + groupidString);
//
//		String[] CountryCodeArray = { "IN" };
//		List<String> CountryCodeList = Arrays.asList(CountryCodeArray);
//		String CountryCodString = String.join(",", CountryCodeList);
//		System.out.println("Country Code List===> " + CountryCodString);
//
//		String[] EntityDescriptionArray = { "PB Al ansari1" };
//		List<String> EntityDescriptioList = Arrays.asList(EntityDescriptionArray);
//		System.out.println("Entity Description List===> " + EntityDescriptioList);
...
//
//		String[] entityIdArray = { "1204" };
//		List<String> entityIdList = Arrays.asList(entityIdArray);
//		System.out.println("entity Id List===> " + entityIdList);
//		String entityIdListString = String.join(",", entityIdList);
//		System.out.println("Get String Value" + entityIdListString);
//
//		String[] IsActiveArray = { "Y" };
//		List<String> IsActiveList = Arrays.asList(IsActiveArray);
//		System.out.println("Is Active List===> " + IsActiveList);
//		String IsActiveListString = String.join(",", IsActiveList);
...
//
		...
		verifyBySelectDropDown(selectEntityDescription, entitDescription);
		verifyBySelectDropDown(selectEntityTypeId, splitvaluesofCorporateDropDownValues);
...
// select one by one of Select in View page of Maker
	...
	public void verifyTheModifiedPage() {
		...
		log.info(" Get the Text of Modify Ppage===>>" + verifyMODIFYGROUPCOUNTRYENTITYTYPEMAKERPage.getText());
		log.info(" verify the Modify  Maker Ppage");
		Assert.assertEquals(verifyMODIFYGROUPCOUNTRYENTITYTYPEMAKERPage.getText(),
				"MODIFY GROUP COUNTRY ENTITY TYPE MAKER");
	}
	...
	public void selectCOrporateTypeDropDownOnModifyPage() {

//		String[] CorporateTypeId = { "1282-entity" };
//		List<String> CorporateTypeIdList = Arrays.asList(CorporateTypeId);
//		System.out.println("Corporate Type IdList===> " + CorporateTypeIdList);
//		String CorporateTypeIdString = String.join(",", CorporateTypeIdList);
//		System.out.println("Get Corporate Type Id String===>>" + CorporateTypeIdString);
//		
		...
		for (int i = 0; i < AllselectentityTypeIdInModify.size(); i++) {
			...
			if (splivaluesCorporateTypedropdown.equals(splitvaluesofCorporateDropDownValues)) {
				log.info("drop down Value Has Matched with Eentity Type id ==>"+splitvaluesofCorporateDropDownValues);
                 // GetTextOfAllDropDownValues OR selectByindex 
				selectentityTypeIdInModify.selectByVisibleText(GetTextOfAllDropDownValues);
				System.out.println("selected entity Type Id In Modify Page Drop Down ==>>"+splitvaluesofCorporateDropDownValues);
				break;
			} else {
				System.out.println("Not selected entity Type Id In Modify Page Drop Down ==>>"+splitvaluesofCorporateDropDownValues);
			}
		}
	...
	// verification Message In Modify Group Country Entity Type Maker Page
	public void verificationMessageInModifyGroupCountryEntityTypeMakerPage() {
	...
	//
	public void verifyGroupCountryEntityTypecheckerPageModificationhasDoneByMaker()
			throws InterruptedException {
//		String[] groupid = { "RGEX", "hhj" };
//		List<String> groupidList = Arrays.asList(groupid);
//		System.out.println("Get group id List===> " + groupidList);
//		String groupidListString = String.join(",", groupidList);
//		System.out.println("Get group id List String===>>" + groupidListString);
//
//		String[] CountryCodeArray = { "" };
//		List<String> CountryCodeList = Arrays.asList(CountryCodeArray);
//		System.out.println("Country Code List===> " + CountryCodeList);
//		String CountryCodeString = String.join(",", CountryCodeList);
//		System.out.println("Get Country Code String===>>" + CountryCodeString);
//
//		String[] entityIdArray = { "1204", "hhj" };
//		List<String> entityIdList = Arrays.asList(entityIdArray);
//		System.out.println("entity Id List===> " + entityIdList);
//		String entityIdString = String.join(",", entityIdList);
//		System.out.println("Get entity Id String===>>" + entityIdString);
//
//		String[] IsActiveArray = { "Y", "Y" };
//		List<String> IsActiveList = Arrays.asList(IsActiveArray);
//		System.out.println("Is Active List===> " + IsActiveList);
//		String IsActiveString = String.join(",", IsActiveList);
//		System.out.println("Get Is Active String===>>" + IsActiveString);
		...
		toverifyListOfValuesOnCheckerPageWhenModifiedEntityMapping(EntityTypeID, splitvaluesofCorporateDropDownValues);
		toverifyListOfValuesOnCheckerPageWhenModifiedEntityMapping(IsActive, getIsActiveCountry);
	...
	public void selectThecheckboxOnCheckerPageWhenModifyMakerDaata() {
		//  After Modification Column Value has changed. This is Bug on it.
		  String entityTypeID="//table[@id='list']//tbody/tr/td[6]";
		  selectThecheckboxOnCheckerPageWhenModify(entityTypeID,splitvaluesofCorporateDropDownValues);
	}
...
//refresh Method Should be use when the when Search Text is not Remove and Click on select All  values.
	public void verifyViewpageBySearchFieldWhenModifyTheEntityTypeMapping()throws InterruptedException {
		String[] search = { GroupId, GetTextOfGroupCountry, splitvaluesofCorporateDropDownValues, getIsActiveCountry };
		String searchList = Arrays.toString(search);
		searchList = searchList.substring(1, searchList.length() - 1);
		System.out.println("Get Text of length===>>" + searchList);
		String aa = searchList.trim();
...
//x-path of view page and calling their respective variable  and verify
	public void toVerifyGetTextInViewPageWhenModify() {
		...
		String[] CountryCodeArray = { CountryCode1 };
		List<String> CountryCodeList = Arrays.asList(CountryCodeArray);
		System.out.println("Country Code List ==> " + CountryCodeList);
		...
		String[] IsActiveArray = { getIsActiveCountry };
		List<String> IsActiveList = Arrays.asList(IsActiveArray);
		System.out.println("Is Active List ==> " + IsActiveList);
		...
		verifyViewPage(Groupid, groupidList);
		verifyViewPage(CountryCode, CountryCodeList);
		verifyViewPage(EntityDescription, EntityDescriptioList);
		verifyViewPage(EntityTypeId, entityIdList);
		verifyViewPage(IsActive, IsActiveList);

	}

// 
// verifytheViewMakerPagewithTextwiththierrespectiveColumn
	public static void verifyViewPage(String xpathvalue, List<String> value) {
		List<WebElement> list1 = driver.findElements(By.xpath(xpathvalue));
		int Nooflist = list1.size();
		System.out.println("No of list On View Page ==>" + Nooflist);
		for (int i = 0; i < list1.size(); i++) {
...
//  5 drop down are available on the 
	public void toVerifytheModifyDataInMakerViewPagByUsingDropDown(){
		...
		verifyViewPPageBySelectDropDown(selectGroupId, groupidString);
		verifyViewPPageBySelectDropDown(selectCountryCode, CountryCodString);
		verifyViewPPageBySelectDropDown(selectEntityDescription, EntityDescriptionString);
		verifyViewPPageBySelectDropDown(selectEntityTypeId, entityIdListString);
		verifyViewPPageBySelectDropDown(selectIsActive, IsActiveListString);
